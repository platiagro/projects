...
class TestMonitorings(TestCase):
    def setUp(self):
        ...
        text = (
            ...
            f"readiness_probe_initial_delay_seconds, is_default, parameters, created_at, updated_at) "
            f"VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        )
        conn.execute(text, (TASK_ID, NAME, DESCRIPTION, IMAGE, None, None, TAGS_JSON, dumps([]),
                            EXPERIMENT_NOTEBOOK_PATH, DEPLOYMENT_NOTEBOOK_PATH, "100m", "100m", "1Gi", "1Gi", 300, 0, PARAMETERS_JSON, CREATED_AT, UPDATED_AT,))
