...
TENANT = "anonymous"
EXPECTED_KEYS_IN_PREDICTION_RESPONSE = ["uuid", "deployment_id", "status"]
...
class TestPredictions(TestCase):
    ...
    def setUpClass(cls):
        ...
        text = (
            f"INSERT INTO predictions (uuid, deployment_id, request_body, response_body, status, created_at, updated_at) "
            f"VALUES (%s, %s, %s, %s, %s, %s, %s)"
        )
        conn.execute(
            text,
            (
                ...
                MOCKED_PREDICTION_RESPONSE_BODY,
                "done",
                CREATED_AT,
                UPDATED_AT,
            ),
    ...
    def test_create_prediction(self, mock_requests):
        ...
        for key in EXPECTED_KEYS_IN_PREDICTION_RESPONSE:
    ...
    def test_get_prediction(self):
        ...
        machine_generated = ["uuid", "created_at", "updated_at"]
